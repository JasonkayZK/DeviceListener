...
class LogData(object):
    ...
    def __init__(self):
        ...
        if not os.path.exists(Constant.DATALOG):
            self.makeCache()
            print("Success to create Cache file!")

        if Constant.SYNC:
            self.reloadCache()
            print("成功重载入数据")
    
        with io.open(Constant.DATALOG, mode='r', encoding='utf-8') as f:
    ...
    def updateCache(self):
        ...
        data = dict()
        try:
            fileIn = io.open(Constant.DATALOG, mode='r', encoding='utf-8')
        ...
        fileOut = None
        try:
            fileOut = io.open(Constant.DATALOG, mode='w', encoding='utf-8')
            fileOut.write(unicode(json.dumps(data)))
        except:
            print("Fail to update datalog!")
        finally:
    ...
    def makeCache(self):
        ...
        f = None
        try:
            f = io.open(Constant.DATALOG, mode='w', encoding='utf-8')
            data = {
